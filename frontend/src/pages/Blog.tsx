...
export default function Blog() {
  ...
  useEffect(() => {
    blogApi.list().then(setPosts).finally(() => setLoading(false));
  }, []);
              ...
              <Link key={post.id} to={`/blog/${post.slug}`}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-3 mb-2">
                    ...
                    <span className="text-[11px] text-white/20">{new Date(post.created_at).toLocaleDateString('ko-KR')}</span>
                  </div>
                  <h2 className="font-black text-2xl md:text-3xl tracking-tight group-hover:text-white/80 transition-colors">{post.title}</h2>
                  {post.excerpt && (
                    <p className="text-white/30 text-sm mt-2 line-clamp-2">{post.excerpt}</p>
                  )}
                </div>
