  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 mix-blend-difference">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        <Link to="/" className="font-black text-lg text-white tracking-tighter uppercase">imjemin</Link>
        ...
        <div className="md:hidden bg-black/95 backdrop-blur px-6 py-8 flex flex-col gap-6">
          {links.map(({ to, label }) => (
            <Link key={to} to={to} onClick={() => setOpen(false)}
              className="text-white text-2xl font-black uppercase tracking-tight">{label}</Link>
          ))}
